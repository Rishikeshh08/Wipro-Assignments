...
-- Q2 List all employees along with their manager’s name.
select e.first_name as Employee_name, m.first_name as Mananger_name
from employees e
left join employees m on e.manager_id = e.employee_id;

-- Q3 Find the names of employees who work in the same department as ‘Nancy Greenberg’.
...
-- Q4. Show employees who work in the same city as their manager.
...
-- Q5. List all employees whose job title is the same as their manager’s.
select e.employee_id
from employees e
join employees m on e.manager_id = m.employee_id
join jobs ej on e.job_id = ej.job_id
join jobs mj on m.job_id = mj.job_id
where ej.job_title = mj.job_title;
...
-- Q1 Display the average salary for each job title, only for jobs with more than 3 employees.
...
-- Q2 Find the department that has the highest total salary.
...
-- 3. List each location’s total employee count and average salary.
select l.city, count(e.employee_id) as noOfEmp, avg(e.salary) as AvgSalary
from employees e
join departments d on e.department_id = d.department_id
join locations l on d.location_id = l.location_id
group by l.city;
...
-- Q2. Display employees whose salary is greater than all salaries of employees in the 'IT' department.
...
/*
this is wrong , logic wise correct but there will be no departments with 
no zero employees in employees table , it will be in department table .
...
-- Q4. Display all employees hired after the employee with the highest salary was hired
select e.employee_id, concat(e.first_name, ' ', e.last_name) as EmpName
from employees e
where e.hire_date > (
	select max(ee.hire_date)
    from employees ee
    where ee.salary = (
		select max(salary)
        from employees eee
